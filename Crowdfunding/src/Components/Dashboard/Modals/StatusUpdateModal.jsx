        ...
        <AnimatePresence>
            {statusModal.show && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-2">
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setStatusModal({ ...statusModal, show: false })} className="absolute inset-0 bg-text-primary/80 backdrop-blur-sm" />
                    <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl relative z-10 overflow-hidden" >
                        <div className="p-5">
                            ...
                            <h2 className="text-3xl font-bold text-text-primary uppercase tracking-tighter mb-2">
                                {statusModal.type === 'rejected' ? 'Decline Request' : 
                                 statusModal.type === 'approved' ? 'Approve Withdrawal' : 'Finalize Transfer'}
                            </h2>
                            ...
                            {statusModal.type === 'rejected' && (
                                <div className="space-y-2 mb-4">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-2">Reason for Rejection (Mandatory)</label>
                                    <textarea 
                                        required 
                                        className="w-full bg-gray-50 border-2 border-transparent focus:border-red-500 outline-none px-3 py-4 rounded-2xl font-semibold transition-all min-h-[100px]" 
                            ...
                            {(statusModal.type === 'approved' || statusModal.type === 'completed') && (
                                <div className="space-y-6 mb-4">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-2">Admin Comment (Optional)</label>
                                        <textarea 
                                            className="w-full bg-gray-50 border-2 border-transparent focus:border-primary outline-none px-3 py-4 rounded-2xl font-semibold transition-all min-h-[80px]" 
                            ...
                            <div className="flex gap-4">
                                <button onClick={() => setStatusModal({ ...statusModal, show: false })} className="flex-grow bg-gray-100 text-gray-600 py-5 rounded-2xl font-bold text-lg hover:bg-gray-200 transition-all">CANCEL</button>
                                <button 
                                    ...
                                    className={`flex-[2] py-5 rounded-2xl font-bold text-lg text-white transition-all shadow-xl disabled:opacity-50 ${
