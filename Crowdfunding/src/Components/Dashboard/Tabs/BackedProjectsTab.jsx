    ...
    return (
        <motion.div key="backed-projects" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            {Array.from(new Set(data.donations.map(d => d.campaignId?._id))).map(campaignId => {
                ...
                return (
                    <div key={campaignId} className="bg-white rounded-[40px] border border-gray-100 shadow-xl shadow-text-primary/5 overflow-hidden group">
                        <div className="p-4 md:p-5 flex flex-col md:flex-row gap-10">
                            <img src={campaign.thumbnail} className="w-full md:w-64 h-64 md:h-48 rounded-[32px] object-cover" alt="" />
                            <div className="flex-grow space-y-6">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h3 className="text-3xl font-bold text-text-primary uppercase tracking-tighter">{campaign.title}</h3>
                                        <p className="text-gray-400 font-semibold uppercase tracking-widest text-xs">By {campaign.creatorId?.name}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Total Contributed</p>
                                        <p className="text-3xl font-bold text-primary">${totalContributed}</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                                        <Rocket size={14} className="text-primary" /> Reward Tiers Reached
                                    </p>
                                    <div className="flex flex-wrap gap-3">
                                        {campaign.rewardTiers?.filter(tier => totalContributed >= tier.minimumAmount).map(tier => {
                                            ...
                                            return (
                                                <div key={tier._id} className="bg-gray-50 px-3 py-4 rounded-2xl border border-gray-100 flex items-center justify-between gap-4">
                                                    <div>
                                                        <p className="font-bold text-text-primary text-sm">{tier.title}</p>
                                                    ...
                                                    {isFulfilled && !isConfirmed && (
                                                        <button 
                                                            ...
                                                            className="bg-cream text-text-primary px-4 py-2 rounded-xl font-bold text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-md"
                                                        >
                                        ...
                                        {campaign.rewardTiers?.filter(tier => totalContributed >= tier.minimumAmount).length === 0 && (
                                            <p className="text-xs font-semibold text-gray-400 italic">No tiers reached yet</p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="pt-6 border-t border-gray-50 grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Project Progress</p>
                                        <div className="flex items-center gap-4">
                                            <div className={`p-2 rounded-2xl ${isCampaignEnded ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'}`}>
                                                {isCampaignEnded ? <CheckCircle size={24} /> : <Clock size={24} />}
                                            </div>
                                            <div>
                                                <p className="font-bold text-text-primary uppercase text-sm">{isCampaignEnded ? 'Campaign Ended' : 'Funding Period'}</p>
                                    ...
                                    {isCampaignEnded && (
                                        <div>
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Fulfillment Status</p>
                                            <div className="flex items-center gap-4">
                                                <div className="p-2 bg-cream/10 text-primary rounded-2xl">
                                                    <Rocket size={24} />
                                                </div>
                                                <div>
                                                    <p className="font-bold text-text-primary uppercase text-sm">
            ...
            {data.donations.length === 0 && (
                <div className="py-10 text-center bg-white rounded-[40px] border border-dashed border-gray-200">
                    <p className="text-gray-400 font-semibold uppercase tracking-widest">No backed projects found</p>
                </div>
            )}
        </motion.div>
    );
