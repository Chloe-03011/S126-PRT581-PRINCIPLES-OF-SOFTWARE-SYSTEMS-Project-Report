...
const CreateCampaign = () => {
    ...
    const addTier = () => {
        if (!currentTier.title || !currentTier.minimumAmount) return;
        setFormData(prev => ({
            ...prev,
            ...
            <div className="max-w-4xl mx-auto px-4 -mt-24">
                <div className="bg-white rounded-[40px] shadow-2xl border border-gray-100 p-8 md:p-12">
                            ...
                            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                <div className="grid md:grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        ...
                                        <input name="title" value={formData.title} onChange={handleInputChange} className="w-full bg-gray-50 border-2 border-transparent focus:border-custom-yellow outline-none py-4 px-6 rounded-2xl font-bold transition-all" placeholder="Enter a catchy title" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase text-gray-400 tracking-widest flex items-center gap-2">Category</label>
                                        <select name="category" value={formData.category} onChange={handleInputChange} className="w-full bg-gray-50 border-2 border-transparent focus:border-custom-yellow outline-none py-4 px-6 rounded-2xl font-bold transition-all">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input name="tagline" value={formData.tagline} onChange={handleInputChange} className="w-full bg-gray-50 border-2 border-transparent focus:border-custom-yellow outline-none py-4 px-6 rounded-2xl font-bold transition-all" placeholder="One sentence description" />
                                </div>
                                <div className="grid md:grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        ...
                                        <input type="number" name="fundingGoal" value={formData.fundingGoal} onChange={handleInputChange} className="w-full bg-gray-50 border-2 border-transparent focus:border-custom-yellow outline-none py-4 px-6 rounded-2xl font-bold transition-all" placeholder="5000" />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <input type="date" name="deadline" value={formData.deadline} onChange={handleInputChange} className="w-full bg-gray-50 border-2 border-transparent focus:border-custom-yellow outline-none py-4 px-6 rounded-2xl font-bold transition-all" />
                                    </div>
                                </div>
                        ...
                        {step === 2 && (
                            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                <div className="space-y-2">
                                    ...
                                    <input name="thumbnail" value={formData.thumbnail} onChange={handleInputChange} className="w-full bg-gray-50 border-2 border-transparent focus:border-custom-yellow outline-none py-4 px-6 rounded-2xl font-bold transition-all" placeholder="https://example.com/image.jpg" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-xs font-black uppercase text-gray-400 tracking-widest flex items-center gap-2">Project Story</label>
                                    <textarea name="description" value={formData.description} onChange={handleInputChange} rows="8" className="w-full bg-gray-50 border-2 border-transparent focus:border-custom-yellow outline-none py-4 px-6 rounded-2xl font-bold transition-all resize-none" placeholder="Explain your vision in detail..." />
                                </div>
                            </motion.div>
                        )}
                            ...
                            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                                ...
                                <div className="p-8 border-2 border-dashed border-gray-200 rounded-[30px] space-y-6">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-6">
                                        <input value={currentTier.title} onChange={(e) => setCurrentTier({...currentTier, title: e.target.value})} className="bg-gray-50 border-none outline-none py-3 px-4 rounded-xl font-bold" placeholder="Tier Title (e.g. Early Bird)" />
                                        <input type="number" value={currentTier.minimumAmount} onChange={(e) => setCurrentTier({...currentTier, minimumAmount: e.target.value})} className="bg-gray-50 border-none outline-none py-3 px-4 rounded-xl font-bold" placeholder="Min. Amount ($)" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Reward Type</label>
                                        <select value={currentTier.type} onChange={(e) => setCurrentTier({...currentTier, type: e.target.value})} className="w-full bg-gray-50 border-none outline-none py-3 px-4 rounded-xl font-bold">
                                            <option value="physical">Physical Product</option>
                                            <option value="email">Digital / Email Reward</option>
                                        </select>
                                    </div>
                                    <textarea value={currentTier.description} onChange={(e) => setCurrentTier({...currentTier, description: e.target.value})} className="w-full bg-gray-50 border-none outline-none py-3 px-4 rounded-xl font-bold resize-none" placeholder="What do they get?" rows="3" />
                                    <div className="flex items-center gap-4">
                                        ...
                                        {currentTier.isLimited && (
                                            <input type="number" value={currentTier.totalSlots} onChange={(e) => setCurrentTier({...currentTier, totalSlots: e.target.value})} className="bg-gray-50 border-none outline-none py-2 px-4 rounded-xl font-bold w-32" placeholder="Qty" />
                                        )}
                                    </div>
                    ...
                    <div className="mt-12 pt-12 border-t border-gray-100 flex justify-between gap-4">
                        ...
                        {step < 4 ? (
                            <button onClick={() => setStep(s => s + 1)} className="bg-custom-black text-custom-white flex items-center gap-2 font-black text-sm uppercase tracking-widest px-8 py-4 rounded-2xl hover:bg-gray-900 transition-all active:scale-95 shadow-xl shadow-black/10">
